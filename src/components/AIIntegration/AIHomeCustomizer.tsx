  ...
  const handleStartProcessing = async (photoData: string) => {
    ...
    try {
      if (!process.env.GEMINI_API_KEY) {
        throw new Error('未检测到 Gemini API Key，请确保已正确配置。');
      }
      ...
      console.log('Calling Nano Banana (Gemini 2.5) for high-fidelity fusion...');
      
      const fusionPromise = ai.models.generateContent({
        ...
        contents: {
          parts: [
            ...
            {
              inlineData: {
                data: tableRenderBase64,
                mimeType: 'image/jpeg'
              }
            },
            {
              text: `TASK: HIGH-FIDELITY ARCHITECTURAL FUSION.
      ...
      // 4. Execution with timeout racing
      const response = await Promise.race([fusionPromise, timeoutPromise]) as any;

      console.log('AI response received');
      
      if (!response) throw new Error('AI 返回内容为空');

      let responseText = '';
      try { responseText = response.text || ''; } catch(e) { /* ignore text errors */ }
      ...
      if (newImage) {
        ...
        if (responseText && (responseText.includes('安全') || responseText.includes('sorry'))) {
          throw new Error('AI 安全策略限制生成该融合图。请尝试上传更开阔、中性的照片。');
        }
        throw new Error('AI 未能生成有效的合成图像。请确保照片中包含足够宽敞的地面。');
      }
    } catch (error) {
            ...
            <div className="flex flex-row md:flex-col items-center md:items-start gap-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs md:text-sm text-slate-500 font-medium leading-relaxed opacity-90">
                  利用 Nano Banana (Gemini 2.5) 模型，自动寻找家中最佳点位并精准融合您的设计。
                </p>
              </div>
            </div>
